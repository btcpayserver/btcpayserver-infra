...
* Wordpress Blog and Merch store.  ([blog.btcpayserver.org](https://blog.btcpayserver.org))  ([store.btcpayserver.org](https://store.btcpayserver.org))
## How to use
...
WOOCOMMERCE_HOST=store.btcpayserver.org
TELEGRAM_TOKEN=...
```
Where: 
...
* `WOOCOMMERCE_HOST` is the store domain name


This assume that `SLACK_HOST` and `MATTERMOST_HOST` and `WOOCOMMERCE_HOST` are pointing to your server.
Then you can run:
